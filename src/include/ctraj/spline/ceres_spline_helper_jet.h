...
namespace ns_ctraj {
    ...
    struct CeresSplineHelperJet {
        ...
        static inline void EvaluateLie(T const *const *sKnots, const T u, const double inv_dt,
            ...
            if (vel_out || accel_out || jerk_out) {
                ...
                if (accel_out || jerk_out) {
                    ...
                    ddCoeff = inv_dt * inv_dt * CeresSplineHelperJet<T, N>::cumulative_blending_matrix_ * p;

                    if (jerk_out) {
                        CeresSplineHelperJet<T, N>::template BaseCoeffsWithTime<3>(p, u);
                        dddCoeff =
                                inv_dt * inv_dt * inv_dt * CeresSplineHelperJet<T, N>::cumulative_blending_matrix_ * p;
                    }
                }
            }
                    ...
                    if (accel_out || jerk_out) {
                        ...
                        if (jerk_out) {
                            ...
                            rot_jerk += dddCoeff[i + 1] * delta + Group::lieBracket(
                                    ddCoeff[i + 1] * rot_vel + T(2) * dCoeff[i + 1] * rot_accel -
        ...
        static inline void
        Evaluate(T const *const *sKnots, const T u, const double inv_dt, Eigen::Matrix<T, DIM, 1> *vec_out) {
            if (!vec_out) return;
    ...
    const typename CeresSplineHelperJet<T, N>::MatN
            CeresSplineHelperJet<T, N>::base_coefficients_ = ns_ctraj::ComputeBaseCoefficients<N, T>();

    template<class T, int N>
    const typename CeresSplineHelperJet<T, N>::MatN
            CeresSplineHelperJet<T, N>::blending_matrix_ = ns_ctraj::ComputeBlendingMatrix<N, T, false>();

    template<class T, int N>
    const typename CeresSplineHelperJet<T, N>::MatN
            CeresSplineHelperJet<T, N>::cumulative_blending_matrix_ = ns_ctraj::ComputeBlendingMatrix<N, T, true>();
